...
{	double amplitude ;
	...
	double start_freq, end_freq, samples ;
...
main (int argc, char * argv [])
{
	...
	params.samples = parse_double_or_die (argv [argc - 2], "samples") ;
	filename = argv [argc - 1] ;

	check_int_range ("sample rate", params.samplerate, 1000, 200 * 1000) ;
	check_double_range ("samples", params.samples, 1, 1000000000000) ;
...
usage_exit (const char * argv0)
{
	...
	printf ("\nUsage :\n\n    %s  [options] <sample rate> <length in samples> <sound file>\n\n", progname) ;
...
static void
write_chirp (SNDFILE * file, int samplerate, double samples, double amp, double w0, double w1, freq_func_t sweep_func)
{
	double instantaneous_w, current_phase ;
	float * data ;
	int total_samples, k ;

	total_samples = lrint (samples ) ;
...
generate_file (const char * filename, const PARAMS * params)
{
	...
	write_chirp (file, params->samplerate, params->samples, params->amplitude, w0, w1, params->sweep_func) ;
