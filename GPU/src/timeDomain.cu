...
float *TDconvolution(float ** d_ibuf, float ** d_rbuf, long long size, long long old_size, long long oFrames){
	...
	preliminaryTimeDomainConvolution<<<1, tileSize>>> (*d_ibuf, *d_rbuf, d_obuf);
	int nStreams = 4;
	cudaStream_t stream[nStreams];
		for (int i = 0; i < nStreams; ++i) {
			checkCudaErrors(cudaStreamCreate(&stream[i]));
		}
	long long newSize = oFrames - 32;
	long long offset = newSize / nStreams;
	int numThreads = 512;
	int numBlocks = ( offset + numThreads - 1) / numThreads;
	for(int i = 0; i < nStreams; i++){
		//timeDomainConvolutionStreamed<<<numBlocks, numThreads, 0, stream[i]>>> (*d_ibuf, *d_rbuf, d_obuf, old_size, oFrames - old_size + 1, offset * (i + 1));
		
	}
	numBlocks = (newSize + numThreads - 1) / numThreads;
	timeDomainConvolution<<<numBlocks, numThreads>>> (*d_ibuf, *d_rbuf, d_obuf, old_size, oFrames - old_size + 1);
	...
	fprintf(stderr, "Scaling signal according to input\n");
	RealFloatScale <<< numBlocks, blockSize >>> (d_obuf, oFrames, scale);
